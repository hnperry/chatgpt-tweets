...
```
Let's also check the distribution of sentiments
...
neutral     55487
```
![sentimentdist](https://user-images.githubusercontent.com/116209783/234078086-08eed57c-0b82-411c-bdf3-89183012be53.png)

Looks like there are a lot of negative tweets about chatGPT!
...
We can also explore word frequency by calculating the frequency distribution of the words and plotting it as a bar plot. 
...
![wordcounts](https://user-images.githubusercontent.com/116209783/234063581-74966e8c-033b-4e51-97ee-3674c5778cd1.png)


#### 1.3.2 Sentence Length
...
### 1.4 Bag of Words TFIDF Vectorization

TFIDF Vectorization (Term Frequency-Inverse Document Frequency) is a text vectorization technique that transforms text data into numerical features. The term frequency (TF) portion accounts for how common a word is, while the inverse document frequency (IDF) assigns a weight to each word, aiming to reduce the weight of the word if it occurs in many tweets. For example, we see above that chatGPT is the most common word among these tweets. Although it is the most common word, does it really tell us anything about the sentiment of a given tweet?

First, we need to split our data into features and targets. From there, we can use sklearn's train test split to prepare training and testing sets.
...
```
Now data is portioned into the default 75%/25% split between training and testing sets.
```
(164470,) (164470,)
(54824,) (54824,)
```
